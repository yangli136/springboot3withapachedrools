...
public class HelloWorldUnit implements RuleUnitDefinition {
    ...
    public void defineRules(RulesFactory rulesFactory) {
        ...
        rulesFactory.rule().on(strings).filter(EQUAL, "Hello World") // when no extractor is provided "this" is implicit
                .execute(results, r -> r.add("greeting matches [Hello World]")); // the consequence can ignore the matched facts
        ...
        rulesFactory.rule().on(strings) // since the datasource has been already initialized its class can be inferred without the need of explicitly passing it
                ...
                .execute(results, (r, s) -> r.add("greeting length is greater than 5:" + s.toUpperCase())); // this consequence also uses the matched fact
        ...
        rulesFactory.rule("MyRule") // it is possible to optionally set a name for the rule
                ...
                .execute(results, r -> r.add("greeting length is less than 5."));
